...
vi.mock('../../backend/lambdas/letter-processor/pdf-utils.js', () => ({
  mergeFiles: vi.fn().mockResolvedValue(Buffer.from('merged-pdf'))
}))

vi.mock('../../backend/lambdas/letter-processor/gemini.js', () => ({
...
describe('Letter Processor', () => {
  ...
  it.skipIf(!hasAwsCredentials)('should process files and save draft (integration)', async () => {
    const { handler } = require('../../backend/lambdas/letter-processor/index')
  ...
  it('should export handler function', () => {
    const letterProcessor = require('../../backend/lambdas/letter-processor/index')
    expect(typeof letterProcessor.handler).toBe('function')
  })

  it('should have pdf-utils module', () => {
    const pdfUtils = require('../../backend/lambdas/letter-processor/pdf-utils')
    expect(typeof pdfUtils.mergeFiles).toBe('function')
  })

  it('should have gemini module', () => {
    const gemini = require('../../backend/lambdas/letter-processor/gemini')
